...
interface SiteDetails {
  ...
  fsr: string
  lotCoveragePct: string
  topography: 'flat' | 'slope' | ''
  boulevardTrees: boolean
  propertyLineTrees: boolean
}
...
function StepSiteDetails({ data, onChange }: { data: SiteDetails; onChange: (d: SiteDetails) => void }) {
  const set = (key: keyof SiteDetails) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
    onChange({ ...data, [key]: (e.target as HTMLInputElement).type === 'checkbox' ? (e.target as HTMLInputElement).checked : e.target.value })
  ...
  return (
    <div className="grid sm:grid-cols-2 gap-5">
      ...
      <Field label="FSR" hint="floor space ratio">
        <input className={inputCls} type="number" min="0" max="10" step="0.1" value={data.fsr} onChange={set('fsr')} placeholder="e.g. 0.9" />
      </Field>
      <Field label="Lot Coverage" hint="% max">
        <input className={inputCls} type="number" min="0" max="100" step="1" value={data.lotCoveragePct} onChange={set('lotCoveragePct')} placeholder="e.g. 45" />
      </Field>

      {/* Topography */}
      <div className="sm:col-span-2">
        <p className="font-sans text-sm font-medium text-fort-charcoal mb-2">Lot Topography</p>
        <div className="flex gap-3">
          {(['flat', 'slope'] as const).map(t => (
            <button
              key={t}
              type="button"
              onClick={() => onChange({ ...data, topography: t })}
              className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-lg border font-sans text-sm font-medium transition-colors duration-150 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-fort-gold ${
                data.topography === t
                  ? 'bg-fort-charcoal text-white border-fort-charcoal'
                  : 'bg-white text-fort-gray border-fort-charcoal/20 hover:border-fort-charcoal/40'
              }`}
            >
              {t === 'flat' ? (
      ...
      {/* Trees */}
      <div className="sm:col-span-2">
        <p className="font-sans text-sm font-medium text-fort-charcoal mb-2">Trees on Site</p>
        <div className="flex flex-col gap-2">
          {([
            { key: 'boulevardTrees', label: 'Boulevard trees', hint: 'City-owned trees in the road allowance — may restrict excavation and utility connections' },
            { key: 'propertyLineTrees', label: 'Property line trees', hint: 'Trees on or straddling the property line — may require arborist report and protection plan' },
          ] as const).map(({ key, label, hint }) => (
            <label key={key} className="flex items-start gap-3 p-3 rounded-lg border border-fort-charcoal/15 bg-white hover:border-fort-charcoal/30 transition-colors duration-150 cursor-pointer">
              <input
                type="checkbox"
                checked={data[key]}
                onChange={set(key)}
                className="mt-0.5 w-4 h-4 rounded border-fort-charcoal/30 accent-fort-gold cursor-pointer"
              />
              <div>
                <p className="font-sans text-sm font-medium text-fort-charcoal">{label}</p>
                <p className="font-sans text-xs text-fort-gray mt-0.5 leading-relaxed">{hint}</p>
              </div>
            </label>
          ))}
        </div>
...
function StepCostInputs({ data, onChange }: { data: CostInputs; onChange: (d: CostInputs) => void }) {
  ...
  const design = parseFloat(data.designCosts) || 0
  const devFeePct = parseFloat(data.developerFeePct) || 0
  ...
  const total = land + hard + soft + contingency + design + devFee
  ...
  return (
    <div className="grid sm:grid-cols-2 gap-5">
      ...
      <Field label="Design Costs" hint="$ (architecture, engineering, permits)">
        <input className={inputCls} type="number" min="0" value={data.designCosts} onChange={set('designCosts')} placeholder="e.g. 120000" />
      </Field>
      <Field label="Developer Fee" hint="% of total project cost">
        <input className={inputCls} type="number" min="0" max="20" step="0.5" value={data.developerFeePct} onChange={set('developerFeePct')} placeholder="e.g. 5" />
      </Field>
...
function StepRevenue({ data, onChange }: { data: RevenueInputs; onChange: (d: RevenueInputs) => void }) {
  ...
  const unitTypes = [
    ...
    { key: 'fourBeds', priceKey: 'fourBedPricePer', label: '4 Bedroom', sqft: 1300 },
  ] as const

  const avgSqft = parseFloat(data.avgUnitSqft) || 700

  function calcRev(count: string, price: string): number {
    const n = parseInt(count) || 0
    const p = parseFloat(price) || 0
    if (n === 0 || p === 0) return 0
    return data.priceMode === 'per_unit' ? n * p : n * p * avgSqft
  }

  const totalUnits =
    (parseInt(data.studios) || 0) + (parseInt(data.oneBeds) || 0) +
    (parseInt(data.twoBeds) || 0) + (parseInt(data.threeBeds) || 0) +
    (parseInt(data.fourBeds) || 0)

  const totalRevenue =
    calcRev(data.studios, data.studioPricePer) +
    calcRev(data.oneBeds, data.oneBedPricePer) +
    calcRev(data.twoBeds, data.twoBedPricePer) +
    calcRev(data.threeBeds, data.threeBedPricePer) +
    calcRev(data.fourBeds, data.fourBedPricePer)
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-lg border border-fort-charcoal/15 overflow-hidden">
        <div className="grid grid-cols-3 px-4 py-2 bg-fort-charcoal/5 border-b border-fort-charcoal/10">
          <p className="font-sans text-xs font-semibold text-fort-gray uppercase tracking-wider">Units</p>
      ...
      {totalRevenue > 0 && (
        <div className="rounded-lg bg-emerald-50 border border-emerald-200 p-4">
          <p className="font-sans text-xs font-semibold text-emerald-800 uppercase tracking-wider mb-3">Revenue Preview</p>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="font-sans text-xs text-fort-gray">Total Units</p>
              <p className="font-sans font-semibold text-sm text-fort-charcoal">{totalUnits}</p>
            </div>
            <div>
              <p className="font-sans text-xs text-fort-gray">Total Revenue</p>
              <p className="font-sans font-semibold text-sm text-fort-charcoal">
...
const defaultForm: FormData = {
  site: { address: '', lotSizeSqft: '', zoning: '', currentUse: '', proposedUnits: '', fsr: '', lotCoveragePct: '', topography: '', boulevardTrees: false, propertyLineTrees: false },
  costs: { landValue: '', hardCostPerSqft: '', grossFloorAreaSqft: '', softCostPct: '15', contingencyPct: '10', designCosts: '', developerFeePct: '5' },
  revenue: { studios: '0', studioPricePer: '', oneBeds: '0', oneBedPricePer: '', twoBeds: '0', twoBedPricePer: '', threeBeds: '0', threeBedPricePer: '', fourBeds: '0', fourBedPricePer: '', priceMode: 'per_unit', avgUnitSqft: '' },
